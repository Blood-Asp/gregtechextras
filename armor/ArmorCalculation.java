...
import static gregapi.data.TD.Properties.DONT_SHOW_THIS_COMPONENT;
	...
	public static float[] calculateArmor(ItemStack[] parts) {
		...
		if (parts != null) {
			...
			for (int i = 0; i < parts.length; i++) {
				if (parts[i] != null) {
					OreDictItemData data = OM.data(parts[i]);
					if (data != null && (data.mPrefix == OP.plate || data.mPrefix == OP.plateGem || data.mPrefix == OP.plateAlloy)) {
						// Weight
						def[0] = def[0] + Values.INSTANCE.getValues(data.mMaterial.mMaterial).weight;
						if (data.mPrefix == OP.plateAlloy && data.mMaterial.mMaterial == MT.Iridium) {
							def[0] = def[0] - 20;
						}
						// physicalDef
						...
						if (data.mPrefix == OP.plate && data.mMaterial.mMaterial == MT.Rubber) {
							def[7] = def[7] + 0.25f;
							def[9] = def[9] + 2.0f;
						}
						if (data.mPrefix == OP.plate && data.mMaterial.mMaterial == MT.Lead) {
							def[6] = def[6] + 0.30f;
						}
						if (data.mPrefix == OP.plate && data.mMaterial.mMaterial == MT.Plastic) {
							def[7] = def[7] + 0.25f;
						}
						if (data.mPrefix == OP.plate && data.mMaterial.mMaterial == MT.NeodymiumMagnetic) {
							def[11] = def[11] + 2.0f;
						}
						if (data.mPrefix == OP.plate && data.mMaterial.mMaterial == MT.NetherStar) {
							def[8] = def[8] + 0.20f;
						}
						if (data.mPrefix == OP.plateGem && data.mMaterial.mMaterial == MT.InfusedFire) {
							def[10] = def[10] + 3.0f;
						}
						if (data.mPrefix == OP.plateGem && data.mMaterial.mMaterial == MT.InfusedEntropy) {
							def[10] = def[10] + 4.0f;
						}
					...
					} else if (parts[i].getItem().equals(IL.Cell_Empty.getItem())) {
						def[15] += 8000;
					...
					} else if (data !=null && data.mPrefix == OP.circuit) {// processing power stuff
						if (data.mMaterial.mMaterial == MT.Basic) {
							def[30] += 100;
						} else if (data.mMaterial.mMaterial == MT.Good) {
							def[30] += 200;
						} else if (data.mMaterial.mMaterial == MT.Advanced) {
							def[30] += 300;
						} else if (data.mMaterial.mMaterial == MT.Data) {
							def[30] += 400;
						} else if (data.mMaterial.mMaterial == MT.Elite) {
							def[30] += 500;
						} else if (data.mMaterial.mMaterial == MT.Master) {
							def[30] += 600;
						}
