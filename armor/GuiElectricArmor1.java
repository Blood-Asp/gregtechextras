...
//		float[] def = cont.mInvArmor.def;

//		this.fontRendererObj.drawString(GTExtras.locTip("weight") + ": " + (int) (def[0]), 5, 5, 0x000000);
//		this.fontRendererObj.drawString(GTExtras.locTip("phydef") + ": " + (Math.round(def[1] * 1000) / 10.0) + "%", 5, 15, 0x000000);
//		this.fontRendererObj.drawString(GTExtras.locTip("prodef") + ": " + (Math.round(def[2] * 1000) / 10.0) + "%", 5, 25, 0x000000);
//		this.fontRendererObj.drawString(GTExtras.locTip("charge") + ": " + formatter.format((int) Math.floor(def[12])) + " EU", 5, 35, 0x000000);
		// this.fontRendererObj.drawString("MagicDef: "+(Math.round(cont.mInvArmor.def[4]*1000)/10.0)+"%",5,45,0x000000);
		// this.fontRendererObj.drawString("ExplosionDef: "+(Math.round(cont.mInvArmor.def[5]*1000)/10.0)+"%",5,55,0x000000);
	...
	protected void drawTooltip(int x, int y) {
		...
		if(x>=7&&y>=11&&x<=33&&y<=17){
				list.add(GTExtras.locTip("weight") + ": " + cont.mInvArmor.data.weight);
		...
		if(tab==0){
			...
			if(x>=7&&y>=22&&x<=70&&y<=28){list.add("Processing Power Provided: "+cont.mInvArmor.data.processingPower);
					...
					if (x >= 28 && x <= 58) {
			if (y >= 25 && y <= 32) {
				list.add(GTExtras.locTip("phydef") + ": " + (Math.round(cont.mInvArmor.data.physicalDef * 1000) / 10.0) + "%");
			} else if (y >= 33 && y <= 41) {
				list.add(GTExtras.locTip("prodef") + ": " + (Math.round(cont.mInvArmor.data.projectileDef * 1000) / 10.0) + "%");
			} else if (y >= 42 && y <= 50) {
				list.add(GTExtras.locTip("firedef") + ": " + (Math.round(cont.mInvArmor.data.fireDef * 1000) / 10.0) + "%");
			} else if (y >= 51 && y <= 59) {
				list.add(GTExtras.locTip("magdef") + ": " + (Math.round(cont.mInvArmor.data.magicDef * 1000) / 10.0) + "%");
			} else if (y >= 60 && y <= 68) {
				list.add(GTExtras.locTip("expdef") + ": " + (Math.round(cont.mInvArmor.data.explosionDef * 1000) / 10.0) + "%");
			}
		} else if (x >= 59 && x <= 90) {
			if (y >= 25 && y <= 32) {
				list.add(GTExtras.locTip("raddef") + ": " + (Math.round(cont.mInvArmor.data.radiationDef * 1000) / 10.0) + "%");
				if(cont.mInvArmor.data.fullRadiationDef){
				list.add("Radiation Immunity");}
			} else if (y >= 33 && y <= 41) {
				list.add(GTExtras.locTip("eledef") + ": " + (Math.round(cont.mInvArmor.data.electricDef * 1000) / 10.0) + "%");
				if(cont.mInvArmor.data.fullElectricDef){
					list.add("Electric Immunity");}
			} else if (y >= 42 && y <= 50) {
				list.add(GTExtras.locTip("whidef") + ": " + (Math.round(cont.mInvArmor.data.witherDef * 1000) / 10.0) + "%");
			} else if (y >= 51 && y <= 59) {
				if(cont.mInvArmor.data.type!=3){
					list.add("Fall Damage absorbtion");
					list.add("Only for Boots");
				}else{
				list.add(GTExtras.locTip("abs1") + " " + (int) Math.round(cont.mInvArmor.data.fallDef) + GTExtras.locTip("abs2"));}
			} else if (y >= 60 && y <= 68) {
				if(x<69){
				list.add(GTExtras.locTip("thorns") + ": " + (int) Math.round(cont.mInvArmor.data.thornsSingle) + " Dmg");
				list.add("Total "+GTExtras.locTip("thorns") + ": " + (int) Math.round(cont.mInvArmor.data.thorns) + " Dmg");
				}else{
				list.add(GTExtras.locTip("magnet") + ": " + cont.mInvArmor.data.magnetSingle + " m");
				list.add("Total "+GTExtras.locTip("magnet") + ": " + cont.mInvArmor.data.magnet + " m");}
			}
		}
		}
		if (!list.isEmpty())
